...
public  class FileUtil {
    ...
    public static final String NGINX_WWW_ROOT = "/user/info";
        ...
        if (files != null && files.size() > 0) {
            for (MultipartFile posterFile : files) {
                ...
                if (!Strings.isNullOrEmpty(name)) {
                    ...
                    String YYYY_MM_DD = DateTime.now().toString("YYYYMMdd");
                    String POSTER_URI = String.format("%s.%s", now, ext);

                    String POSTER_FOLDER = String.format("%s/%s/%s", NGINX_WWW_ROOT, YYYY_MM_DD, "goods");
                    String CHMOD_FOLDER = String.format("%s/%s", NGINX_WWW_ROOT, YYYY_MM_DD);
                    File POSTER_FOLDER_FILE = new File(POSTER_FOLDER);

                    if (!POSTER_FOLDER_FILE.exists()) {
                        POSTER_FOLDER_FILE.setReadable(true);
                        POSTER_FOLDER_FILE.setWritable(true);
                    ...
                    try {
                        ...
                        Runtime.getRuntime().exec("chmod 755 -R " + CHMOD_FOLDER);
                        String poster = String.format("/%s/%s/%s/%s","upload",YYYY_MM_DD,"goods",POSTER_URI);
                        paths.add(poster);
                    } catch (IOException e) {
