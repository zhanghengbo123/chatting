...
function closeShowCity() {
	...
	if (fromCode == "") {
		$("#fromCode").val("简码/汉字");
		from_to_station_class_gray($("#fromCode"));
		$("#fromStation").val("");
	}
	...
	if (toCode == "") {
		$("#toCode").val("简码/汉字");
		from_to_station_class_gray($("#toCode"));
		$("#toStation").val("");
	}
}
...
function clearStation(flag, event) {
	...
	if (flag == 'from') {
		var fromCode = $("#fromCode").val();
		var fromStation = $("#fromStation").val();
		if (fromCode == "" || fromStation == '') {
			$("#fromCode").val('');
			$("#fromStation").val('');
		} else {
			var join = fromCode + '|' + fromStation;
			if (typeof (station_names) != "undefined") {
...
function setFromStationStyle() {
	...
	if (fromCode == "") {
		...
		from_to_station_class_plain($("#fromCode"));
	}
}
...
function setToStationStyle() {
	...
	if (toCode == "") {
		...
		from_to_station_class_plain($("#toCode"));
	}
}
